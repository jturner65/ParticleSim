...
namespace particleSystem {
	...
	public:
		...
		//inline void setPosition(const Eigen::Ref<const Eigen::Vector3d>& _val) { q[0].segment<3>(0) << _val; }
		//inline void setPosition(const Eigen::Ref<const Eigen::Vector3d>& _val, int idx) { q[idx].segment<3>(0) << _val; }

		inline Eigen::Vector3d& getPosition() { return position[0]; }
		inline Eigen::Vector3d& getPosition(int idx) { return position[idx]; }
		inline void setPosition(const Eigen::Ref<const Eigen::Vector3d>& _val) { position[0] << _val; }
		inline void setPosition(const Eigen::Ref<const Eigen::Vector3d>& _val, int idx) { position[idx] << _val; }

		inline Eigen::Vector3d& getVelocity() { return velocity[0]; }
		inline Eigen::Vector3d& getVelocity(int idx) { return velocity[idx]; }
		inline void setVelocity(const Eigen::Ref<const Eigen::Vector3d>& _val) { velocity[0] << _val; qdot[0].segment<3>(0) << _val; q[0].segment<3>(3) << _val; }
		inline void setVelocity(const Eigen::Ref<const Eigen::Vector3d>& _val, int idx) { velocity[idx] << _val; qdot[idx].segment<3>(0) << _val; q[idx].segment<3>(3) << _val;}
	...
	private:
		...
		//void init(const Eigen::Ref<const Eigen::Vector3d>& _pos, const Eigen::Ref<const Eigen::Vector3d>& _velocity, const Eigen::Ref<const Eigen::Vector3d>& _forceAcc) {
		//	//set up deques to have 2 spots
		//	//Eigen::VectorXd tmp(6), tmpDot(6);
