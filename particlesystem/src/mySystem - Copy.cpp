...
namespace particleSystem{
			...
			for (int qIdx = pIdx + 1; qIdx < psize; ++qIdx) {
				...
				if (dist < partColDist) {// check if particles within potential collision dist of each other
					if (checkParticlePairForCollision(p[pIdx], p[qIdx], dist)) {					//check if headed in same direction
						...
						p[pIdx]->getVelocity() = (p[qIdx]->getVelocity());
						p[qIdx]->getVelocity() = (pVel);
	...
	bool mySystem::checkParticlePairForCollision(std::shared_ptr<myParticle> partA, std::shared_ptr<myParticle> partB, double dist){
		...
		double colRad = 2 * partRad;											//minimum distance within which a collision will occur 
		Eigen::Vector3d partAPos = partA->getPosition(), partBPos = partB->getPosition(), partDiffs = partAPos - partBPos;
		//TODO these are spheres, check for center to center distance duh.  sheesh
    ...
    void mySystem::solveImpEuler4MassSpring(){
        ...
        for(int i=0; i<numParts; ++i){     //get all positions, velocities, forces for every particle
			p0.segment(sIdx, 3) = p[i]->getPosition();
			pIdx += 3;
  			v0.segment(sIdx, 3) = p[i]->getVelocity();
			vIdx += 3;
			f0.segment(sIdx, 3) = p[i]->forceAcc[0]; //p[i]->getForceAcc();
			sIdx += 3;
        }
        ...
        Eigen::Vector3d newVel(0,0,0), newPos(0,0,0);
        pIdx = 0;
        vIdx = 0;
    ...
    void mySystem::dispCenterPart(Eigen::Vector3d& dir){
        ...
        Eigen::Vector3d newPos = ctrOrigPos + .01*dir;		//desired new position
        ...
        ctr->getPosition() = (Eigen::Vector3d(ctrOrigPos + (.95*dispVec.norm()) * nDir));//95% toward the closest particle from central spot
    ...
    void mySystem::resetCtrPointMassSpr(){
        int numParts = p.size(), numSprings = spr.size();
		Eigen::Vector3d ctrOrigPos = calcAndSetCOM(0, p.size() - 1), dispVec;
        p[numParts-1]->getPosition() = (ctrOrigPos);
        for(int i =0; i<numSprings; ++i){        spr[i]-> reCalcRestLen();      }   //recalculate rest length
    }
	//find COM of particle configuration
	Eigen::Vector3d mySystem::calcAndSetCOM(int stIdx, int endIdx) {
	...
	void mySystem::calcFluidForceForAllParticles() {
		...
		for (unsigned int pidx = 0; pidx < p.size(); ++pidx) {
			//Eigen::Vector3d pPos = p[pidx]->getPosition();
			...
			if (shakeValMag > .00001) {
				...
				p[pidx]->applyForce(applyShakeVal);// * (colliders[0].getDistFromCenter(p[pidx]->getPosition(), 1)/(1.0 * (snowGlobRad))));
	...
	void mySystem::addShakeForceToFluid(double fMult, const Eigen::Vector3d& msdrgVal0, const Eigen::Vector3d& msdrgVal1) {
		...
		double scZ = 1.0 / sZ;
		for (int zidx = 0; zidx < sZ / 2; ++zidx) {//increase toward center
		...
		for (int zidx = sZ / 2; zidx < sZ; ++zidx) {//decrease toward center
			//tmpVec = Eigen::Vector3d(shakeVal) * scZ;
	...
	double mySystem::calcAndApplyAnkleForce(int pidx) {
		...
		Eigen::Vector3d force(p[pidx]->forceAcc[0]);//force(p[pidx]->getForceAcc());
		double lenFacc = force.norm(),
			lenThet = tmpThet.norm();
		kp = (0 == lenThet ? 0 : lenFacc * 1.1 / lenThet);
		//kp *= 1;
		Eigen::Vector3d newForce = (0 == kp ? Eigen::Vector3d(0, 0, 0) : -kp * lenThet * (force));
		p[pidx]->applyForce(newForce);       //should be 0
		return kp;
	}//calcAndApplyAnkleForce

	//void mySystem::invokeSolverDerivEval_old() {
	...
	void mySystem::invokeSolverDerivEval() {
		...
		//int solverTypeToUse = (usePartSolver ? p[idx]->solveType : solveType);
		for (int idx = 0; idx < numParts; ++idx) {
			tmpStateVec[0] = p[idx]->getPosition();
	...
	bool mySystem::handlePauseDrawClick(int& clickOnPartIDX, const Eigen::Vector3d& mClickLoc, bool idxIs5) {
		...
		while ((!clickOnPart) && (partidx < p.size())) {
			//cout << "part to check click loc : " << evec3dToStr(p[partidx]->getPosition()) << endl;
		...
		if ((!clickOnPart) && (clickOnCnstrnt)) {
			//std::shared_ptr<myParticle> tmpPart1(new myParticle(1.0, Eigen::Vector3d(mClickLoc), Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, 0), RK4));
			//p.push_back(tmpPart1);
			myParticle::ID_gen = p.size();
			...
			clickOnPartIDX = -1;
			flags[buildDragCnstrnt] = false;		//building constraint from a particle to another particle or space (path constraint) - do not make a path constraint if particle is already on a path constraint
			//std::shared_ptr<myParticle> tmpPart(new myParticle(1.0, Eigen::Vector3d(mClickLoc), Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, 0), RK4));
			//p.push_back(tmpPart);
			myParticle::ID_gen = p.size();
			p.push_back(std::make_shared<myParticle>(1.0, Eigen::Vector3d(mClickLoc), Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, 0), RK4));
			sceneModded = true;
	...
	bool mySystem::handlePauseDrawRel(int& clickOnPartIDX, const Eigen::Vector3d& mRelLoc, bool idxIs5) {
		...
		if (flags[buildDragCnstrnt]) {
			while ((!releasedOnPart) && (partidx < p.size())) {
				//cout << "part to check rel loc : " << evec3dToStr(p[partidx]->getPosition()) << endl;
	...
	void mySystem::calcConstraintForces(){
		...
		//VectorXd constraintForce = lambda.transpose() * partialCpartialX;
		//lambda = (solnL.inverse()) * solR3;
	...
	void mySystem::buildCnstrntStruct(bool multiCnstrnt){
		...
		for (unsigned int idx = 0; idx < p.size(); ++idx){
			shared_ptr<myParticle> part = p[idx];
			int mIdx = 0;
			double cnstDist = 99999999999;
			int cnstIDX = -1;
			partLocNext = part->getPosition() + (deltaT *  part->getVelocity()) + (.5 * deltaT * deltaT * part->forceAcc[0]);//approx location of particle next timestep if unconstrained
			//partLocNext = part->getPosition() + (deltaT *  part->getVelocity()) + (.5 * deltaT * deltaT * part->getForceAcc());//approx location of particle next timestep if unconstrained
            //for jumping constraints
			double tmpDist;
			for (unsigned int cIdx = 0; cIdx < numCnstrnts; ++cIdx){
				//if((part->ID == c[cIdx]->p1ID) || ((part->ID != c[cIdx]->p1ID) && (part->ID ==  c[cIdx]->p2ID))){		//if particle is p1 or p2 in constraint cnstr
				if((part->ID == c[cIdx]->p1ID)  || (part->ID ==  c[cIdx]->p2ID)){		//if particle is p1 or p2 in constraint cnstr
			...
			if(cnstIDX == -1) {
				//cout<<"Error calculating closest constraint to particle :"<<part<<" num constraints : "<<numCnstrnts<<endl; 
				if(multiCnstrnt) {continue;}
			}//means particle has no constraint, continue with particle loop

			//for(int idxOff = 0; idxOff < 3; ++idxOff){
			//	tmpq[nIdx+idxOff]		=  part->getPosition()[idxOff];	
			//	tmpqdot[nIdx+idxOff]	=  part->getVelocity()[idxOff];
			//	tmpqdotdot[nIdx+idxOff]		=  part->getForceAcc()[idxOff];
			//}
			tmpq.segment(nIdx, 3) = part->getPosition();
			tmpqdot.segment(nIdx, 3) = part->getVelocity();
			tmpqdotdot.segment(nIdx, 3) = part->forceAcc[0]; //p[i]->getForceAcc();
			...
			for (unsigned int cIdx = 0; cIdx < numCnstrnts; ++cIdx){
				if(cnstIDX == cIdx){ //closest path constraint
					if ((part->ID == c[cIdx]->p1ID)){		//if particle is p1 in constraint cnstr
						//cout<<"\np1 calc:"<<endl;
						tmpVec4	=  c[cIdx]->calcPartialCP1(part, p[c[cIdx]->p2Idx]);	//path constraint
						tmpVec5 =  c[cIdx]->calcPartialCdotP1(part, p[c[cIdx]->p2Idx]);
					} else if((part->ID != c[cIdx]->p1ID) && (part->ID ==  c[cIdx]->p2ID)){		//if particle is p1 in constraint cnstr
						//cout<<"\np2 calc:"<<endl;
						tmpVec4	= c[cIdx]->calcPartialCP2(p[c[cIdx]->p1Idx], part);
						tmpVec5 = c[cIdx]->calcPartialCdotP2(p[c[cIdx]->p1Idx], part);
					}
				} 
				else if((part->ID != c[cIdx]->p2ID) && (part->ID ==  c[cIdx]->p1ID)){		//if particle is p1 in constraint cnstr and not a path constraint
					//cout<<"\np2 calc:"<<endl;
					tmpVec4	= c[cIdx]->calcPartialCP1(part, p[c[cIdx]->p2Idx]);
					tmpVec5 = c[cIdx]->calcPartialCdotP1(part, p[c[cIdx]->p2Idx]);
				}
				else if((part->ID != c[cIdx]->p1ID) && (part->ID ==  c[cIdx]->p2ID)){		//if particle is p2 in constraint cnstr and not a path constraint
					//cout<<"\np2 calc:"<<endl;
					tmpVec4	= c[cIdx]->calcPartialCP2(p[c[cIdx]->p1Idx], part);
					tmpVec5 = c[cIdx]->calcPartialCdotP2(p[c[cIdx]->p1Idx], part);
				}			
				...
				tmpCVal(cIdx) = c[cIdx]->ks * c[cIdx]->calcCVal(p[c[cIdx]->p1Idx],p[c[cIdx]->p2Idx]);		//.8 = ks
				tmpCDotVal(cIdx) = c[cIdx]->kd * c[cIdx]->calcCDotVal(p[c[cIdx]->p1Idx],p[c[cIdx]->p2Idx]);//.8 = kd
				...
				tmpJ.block<1,3>(cIdx, nIdx) = tmpVec4.head<3>();
				tmpJdot.block<1, 3>(cIdx, nIdx) = tmpVec5.head<3>();

				//tmpJ.row(cIdx).segment<3>(nIdx) = tmpVec4.head<3>();
				//tmpJdot.row(cIdx).segment<3>(nIdx) = tmpVec5.head<3>();

				//for(int idxOff = 0; idxOff < 3; ++idxOff){
				//	tmpJ(cIdx, nIdx+idxOff) = (1.0 * tmpVec4[idxOff]);	
				//	tmpJdot(cIdx, nIdx+idxOff) = (1.0 * tmpVec5[idxOff]);	
				//}
			}	
		...
		Eigen::MatrixXd tmpJ2(numCnstrnts, nIdx), tmpJDot2(numCnstrnts, nIdx);			//will then shrink these once we have all 0-impact constraints removed
		...
		if(matLen < numCnstrnts){//if a zero-constraint row existed
			tmpJ = Eigen::MatrixXd(matLen,nIdx);
			tmpJdot = Eigen::MatrixXd(matLen,nIdx);
			tmpCVal = Eigen::VectorXd(matLen);
			tmpCDotVal = Eigen::VectorXd(matLen);

			for(int jidx = 0; jidx < matLen; ++jidx){
				for(int pidx = 0; pidx < nIdx; ++pidx){
					tmpJ(jidx, pidx) = tmpJ2(jidx, pidx);
					tmpJdot(jidx, pidx) = tmpJDot2(jidx, pidx);
				}//for each entry in row
				tmpCVal[jidx] = tmpCVal2[jidx];
				tmpCDotVal[jidx] = tmpCDotVal2[jidx];
			}//for every row of old matrix
